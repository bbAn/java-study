...
정수끼리 나누면 소수점이 그냥 없어진다. 오류는 안 난다.   
```java
      int sum = 253;
      System.out.println(sum / 3);              // 84        틀림
      System.out.println((double) sum / 3);     // 84.333...  맞음
```
(double) 을 어디에 붙이느냐도 봐두세요.   
나누기 전에 붙여야 합니다. 나눈 다음에 붙이면 이미 늦습니다.   

(double)(sum / 3)      ←  84.0     이미 잘린 뒤라 소용없음   
(double) sum / 3       ←  84.333   나누기 전에 바꿈   


% 활용 — 이번 주 문제 대부분이 이걸로 풀립니다
...
순서 주의. += 입니다. =+ 가 아닙니다.   
a =+ 5 라고 쓰면 오류는 안 나는데 전혀 다른 뜻이 됩니다.   
한 번 해보고 뭐가 되는지 확인해보세요.   
